...
void handleRoot() {
  ...
  website = website + generateButton("col-xs-6", "tvpoweroff","TV Power Off", "tvpoweroff");
 // website = website + generateButton("col-xs-3", "sspower","SS Power", "sspower");
...
void setup(void){
  ...
  server.on("/tvpower", [](){
    ...
    irsend.sendNEC(0xB27BE0, 32);
    server.send(200, "text/plain", "TV Power");
  });

  server.on("/tvpoweroff", [](){
    Serial.println("TV power off");
    irsend.sendNEC(0xB29F50, 32);
    server.send(200, "text/plain", "TV Power off");
  });
