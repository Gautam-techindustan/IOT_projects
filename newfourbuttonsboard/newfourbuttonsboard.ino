...
{ 
  ...
  if(mystring == "buttons")
  {
   ...
   if(strlen(object["name"]) == 8 && object["key"] == "1234567890"){
      i = ((Sname[6] - 48)*10) + ((Sname[7] - 49)*1);
      Serial.println(i);
      if(i>=16 && i<20){
        i=i-16;
      Serial.println("I am in IN");
      val[i] = object["status"];
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    //clientId.c_str() 
    if (client.connect("ESP8266Client5", mqttUser, mqttPassword)) {
